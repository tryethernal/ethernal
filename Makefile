start:
	...
	docker compose -f docker-compose.prod.yml --env-file .env.nginx.prod up -d
	@echo "Waiting for backend container to be healthy..."
	@docker compose -f docker-compose.prod.yml --env-file .env.nginx.prod exec backend sh -c 'until nc -z localhost 8888; do sleep 1; done'
	@echo "Running sequelize migrations in backend container..."
	docker compose -f docker-compose.prod.yml --env-file .env.nginx.prod exec backend npx sequelize db:create
	docker compose -f docker-compose.prod.yml --env-file .env.nginx.prod exec backend npx sequelize db:migrate
	docker compose -f docker-compose.prod.yml --env-file .env.nginx.prod exec backend npx sequelize db:seed:all
