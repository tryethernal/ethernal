<template>
    <v-card outlined class="flex-grow-1">
        ...
        <v-card-text v-if="explorer.stripeSubscription">
            ...
            <div>Monthly Transaction Quota: <b>{{ explorer.stripeSubscription.transactionQuota.toLocaleString() }} / {{ explorer.stripeSubscription.stripePlan.capabilities.txLimit.toLocaleString() }}</b> (Resetting {{ moment(explorer.stripeSubscription.cycleEndsAt) | moment('MMM. Do') }})</div>
