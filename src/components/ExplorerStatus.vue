<template>
    <v-container fluid>
            ...
            <v-row class="mt-2">
                <v-col cols="3" v-if="syncStatus || loading">
                    <v-alert :class="`${explorerClass}--text`" :type="explorerClass" border="start" border-color elevation="2">
                        ...
                        <div v-if="syncStatus" class="mt-4 text-h4 text-center" style="text-transform: uppercase;">{{ syncStatus }}</div>
        ...
        <template v-if="isRpcSectionAvailable">
            <h2>RPC</h2>
            <v-row class="mt-2">
                <v-col cols="3" v-if="isRpcReachable !== null && isRpcReachable !== undefined || loading">
                    <v-alert :class="`${rpcClass}--text`" :type="rpcClass" border="start" border-color elevation="2">
                        ...
                        <div v-if="rpcStatus" class="mt-4 text-h4 text-center" style="text-transform: uppercase;">{{ rpcStatus }}</div>
                        <v-skeleton-loader v-else type="list-item"></v-skeleton-loader>
                    </v-alert>
                </v-col>
