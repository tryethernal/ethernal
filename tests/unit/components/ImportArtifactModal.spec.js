...
describe('ImportArtifactModal.vue', () => {
    ...
    it('Should let the user edit the name and ABI when parameters are valid', async (done) => {
    ...
    it('Should warn the user if the ABI is not valid', async (done) => {
        const wrapper = helper.mountFn(ImportArtifactModal);
        await wrapper.setData({ dialog: true });
    ...
    it('Should warn the user if the ABI is not valid json', async (done) => {
        const wrapper = helper.mountFn(ImportArtifactModal);
        await wrapper.setData({ dialog: true });
    ...
    it('Should let the user upload a file containing contract metadata', async (done) => {
        const wrapper = helper.mountFn(ImportArtifactModal);
        await wrapper.setData({ dialog: true });
        const syncContractDataMock = jest.spyOn(helper.mocks.server, 'syncContractData');
    ...
    it('Should warn the user if the file is invalid', async (done) => {
        const wrapper = helper.mountFn(ImportArtifactModal);
        await wrapper.setData({ dialog: true });
        const syncContractDataMock = jest.spyOn(helper.mocks.server, 'syncContractData');
