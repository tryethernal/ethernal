...
`landing/public/llms.txt` is a static file served at `tryethernal.com/llms.txt` per the [llmstxt.org](https://llmstxt.org) spec. It lists all product pages, chain directory pages, integrations, docs, and blog link so LLMs can discover and index Ethernal.

**When adding, removing, or renaming a landing page or chain page, update `landing/public/llms.txt` to match.**

The blog `llms.txt` (`blog/public/llms.txt`) is auto-regenerated by the publish workflow, so no manual updates needed for blog articles.
