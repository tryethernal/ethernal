...
- **Never use absolute URLs for internal links** between apps in this repo (landing, blog, app). Always use relative paths (`/auth?flow=public`, `/blog`, `/pricing`). Caddy handles routing between apps in both dev and production, so relative paths work everywhere. Absolute URLs are only for external services (docs, GitHub, Discord, etc.).
