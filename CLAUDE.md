...
- **Proactive scanner**: `.github/workflows/sentry-scanner.yml` — hourly cron queries all unresolved Sentry issues, Claude evaluates which are actionable, creates GitHub issues (feeding into auto-fix pipeline), auto-resolves transient errors. Limited to 3 issues per scan with 90s stagger between creations to prevent workflow storms.
